...
class UserRepository {
    ...
    async findById(uuid: string): Promise<User>{
        ...
        const query = 'SELECT uuid, username, email, user_type from Usuario where uuid = $1'
        const values = [ uuid ]
    ...
    async getUserByCredentials(email: string, password: string): Promise<User | null>{
        try {
            const query = `
            SELECT uuid, username, email FROM Usuario WHERE email=$1 AND password = crypt($2,'secret-hash');
        `
